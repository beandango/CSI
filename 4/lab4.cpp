...
3. Calculate the Area of a Triangle
    ...
    set area to (base*height)*.5
    return area
4. Get Various Area entries
    geochoice()
    show menu with the various options
    take input, store into variable choice, validate it
    return choice
5. Display
    switch statement for choice within main
    for each case, 1-4, with validated input using validateInput(),
    call corresponding function
    whole thing inside while(true), so it repeats until user
    enters the response whose case returns 0
...
float calculateRectangle()
{
  float length = 0, width = 0;                         // radius of the circle
  bool loopFlag = true;                                // control for validation
  string msg("Invalid input.");                        // invalid message
  do
  {
...
float calculateTriangle()
{
  float base = 0, height = 0;                         // radius of the circle
  bool loopFlag = true;                                // control for validation
  string msg("Invalid input.");                        // invalid message
  do
  {
